...
async function renderAuditActionHub(){
  ...
  document.getElementById('mainView').innerHTML = `<div id="audit-export-card" class="space-y-6"><div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4"><div><h2 class="text-[36px] font-black outfit birds-green uppercase tracking-tight"> Audit Action Hub</h2><p class="text-slate-500 font-bold">Time filters, SLA flags, issue drill-down and closure evidence review.</p>${typeof SharePoint !== 'undefined' && SharePoint.isConfigured() ? '<span class="inline-flex items-center gap-1.5 mt-2 px-3 py-1 bg-emerald-50 border border-emerald-200 rounded-full text-xs font-bold text-emerald-700">✓ SharePoint Connected — Actions auto-sync</span>' : '<span class="inline-flex items-center gap-1.5 mt-2 px-3 py-1 bg-amber-50 border border-amber-200 rounded-full text-xs font-bold text-amber-700">⚠ SharePoint not configured — Local only</span>'}</div><div class="flex flex-wrap gap-2 audit-export-controls"><button onclick="exportAuditActions('filtered')" class="btn-primary">⬇️ Export Current Filter</button><button onclick="exportAuditActions('full')" class="btn-secondary"> Full Export</button><button onclick="exportAuditPDF()" class="btn-secondary"> PDF Summary</button></div></div>
