      ...
      <div id="overview-card" class="bg-transparent p-1">
        ...
        <div class="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div class="card p-6 border-t-4 border-t-amber-500">
            ...
            ${function(){ var now = new Date(); var best = null; ehoData.forEach(function(d){ if(!storeMap.get(d.StoreId)) return; var displayName = (typeof originalStoreNames !== 'undefined' && originalStoreNames.get(d.StoreId)) || d.StoreId; var inspDate = d.inspectionDate || d.ehoVisit || ''; if(!inspDate && typeof window._ehoRatings !== 'undefined'){ var csv = window._ehoRatings.get(displayName.toLowerCase()) || window._ehoRatings.get(d.StoreId.toLowerCase()); if(csv) inspDate = csv.inspectionDate || ''; } if(!inspDate) return; var parsed = parseUKDate(inspDate); if(!parsed || isNaN(parsed.getTime())) return; if(!best || parsed > best.parsed){ var rating = d.ehoRating || (window._ehoRatings ? (window._ehoRatings.get(displayName.toLowerCase()) || window._ehoRatings.get(d.StoreId.toLowerCase()) || {}).rating : '') || ''; best = {store: displayName, rating: rating, date: inspDate, parsed: parsed}; } }); if(!best) return '<p class="text-sm text-slate-500 italic">No EHO visit data available.</p>'; var stars = ''; var n = parseInt(best.rating); if(n > 0) for(var i=0;i<n;i++) stars += '★'; return '<div class="flex items-center justify-between bg-amber-50 border border-amber-100 rounded-lg px-4 py-2"><span class="text-sm font-bold text-slate-800">' + best.store + '</span><span class="text-xs text-slate-500"><span class="text-amber-500">' + stars + '</span> <span class="text-amber-700 font-bold ml-2">' + best.date + '</span></span></div>'; }()}
          </div>
