      ...
      <div id="overview-card" class="bg-transparent p-1">
        ...
        <div class="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div class="card p-6 border-t-4 border-t-amber-500">
            <h3 class="font-black outfit text-amber-800 text-lg uppercase mb-3">Most Recent EHO Visit</h3>
            ${function(){ var now = new Date(); var best = null; function tryGetCsv(name,id){ if(typeof window._ehoRatings === 'undefined') return null; return window._ehoRatings.get((name||'').toLowerCase()) || window._ehoRatings.get((id||'').toLowerCase()) || null; } ehoData.forEach(function(d){ if(!storeMap.get(d.StoreId)) return; var displayName = (typeof originalStoreNames !== 'undefined' && originalStoreNames.get(d.StoreId)) || d.StoreId; var inspDate = d.inspectionDate || d.ehoVisit || ''; var csv = tryGetCsv(displayName, d.StoreId); if(!inspDate && csv) inspDate = csv.inspectionDate || csv.nextDue || ''; if(!inspDate) return; var parsed = parseUKDate(inspDate); if(!parsed || isNaN(parsed.getTime())) return; if(!best || parsed > best.parsed){ var rating = d.ehoRating || (csv ? csv.rating : '') || ''; var dd = ('0'+parsed.getDate()).slice(-2) + '/' + ('0'+(parsed.getMonth()+1)).slice(-2) + '/' + parsed.getFullYear(); best = {store: displayName, rating: rating, date: dd, parsed: parsed}; } }); if(!best){ // Also check CSV-only stores not in ehoData
              if(typeof window._ehoRatings !== 'undefined'){ window._ehoRatings.forEach(function(csv,key){ var parsed = csv.inspectionDate ? parseUKDate(csv.inspectionDate) : null; if(!parsed || isNaN(parsed.getTime())){ parsed = csv.nextDue ? parseUKDate(csv.nextDue) : null; } if(parsed && (!best || parsed > best.parsed)){ var dd = ('0'+parsed.getDate()).slice(-2) + '/' + ('0'+(parsed.getMonth()+1)).slice(-2) + '/' + parsed.getFullYear(); best = {store: csv.name || key, rating: csv.rating || '', date: dd, parsed: parsed}; } }); }
            } if(!best) return '<p class="text-sm text-slate-500 italic">No EHO visit data available.</p>'; var stars = ''; var n = parseInt(best.rating); if(n > 0) for(var i=0;i<n;i++) stars += '★'; return '<div class="flex items-center justify-between bg-amber-50 border border-amber-100 rounded-lg px-4 py-2"><span class="text-sm font-bold text-slate-800">' + best.store + '</span><span class="text-xs text-slate-500"><span class="text-amber-500">' + stars + '</span> <span class="text-amber-700 font-bold ml-2">' + best.date + '</span></span></div>'; }()}
          </div>
